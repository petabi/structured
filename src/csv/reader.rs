...
pub enum FieldParser {
    ...
    Dict(Arc<DashMap<String, (u32, usize)>>),
}
        ...
        let arrays = self
            ...
            .map(|parser| match parser {
                ...
                FieldParser::Binary => BinaryBuilder::with_capacity(0)
                    .expect("fail to build empty array")
                    .build(),
                FieldParser::UInt32(_) | FieldParser::Dict(_) => {
...
mod tests {
    ...
    fn get_test_data() -> (Vec<Vec<u8>>, HashMap<String, (u32, usize)>, Vec<Column>) {
        ...
        let mut c5_dict: HashMap<String, (u32, usize)> = HashMap::new();
        c5_dict.insert("t1".to_string(), (1, 1));
        c5_dict.insert("t2".to_string(), (2, 5));
        c5_dict.insert("t3".to_string(), (7, 1));
        let c6_v: Vec<&[u8]> = vec![
            b"111a qwer",
            b"b",
            b"c",
    ...
    fn parse_records() {
        ...
        let mut enum_max_values: HashMap<usize, AtomicU32> = HashMap::new();
        enum_max_values.insert(
            5,
            AtomicU32::new(labels.len().to_u32().unwrap_or(u32::max_value())),
        );
        let parsers = [
            FieldParser::int64(),
            FieldParser::Utf8,
            ...
            FieldParser::Dict(Arc::new(labels.into_iter().collect())),
            FieldParser::Binary,
        ];
    ...
    #[test]
    fn missing_enum_map() {
        let parsers = [FieldParser::Dict(Arc::new(DashMap::new()))];
